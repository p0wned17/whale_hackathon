...
parser.add_argument("--data_path", default="../CompCars/data/", help="path to dataset")
parser.add_argument(
    "--annotation_path", default="annotation/", help="path to save annotation"
)
...
def main():
    ...
    full_data["class"] = (
        full_data["image_name"].apply(lambda x: x.split("/")[1]).astype(int)
    )
    ...
    train[["image_name", "x_1", "y_1", "x_2", "y_2", "class"]].to_csv(
        args.annotation_path + "train.txt", index=False
    )
    val[["image_name", "x_1", "y_1", "x_2", "y_2", "class"]].to_csv(
        args.annotation_path + "val.txt", index=False
    )

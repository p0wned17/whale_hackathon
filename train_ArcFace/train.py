...
def train(
    ...
    train_iter = tqdm(
        train_loader,
        desc=f"Train epoch {epoch}/{config.train.n_epoch-1}",
        dynamic_ncols=True,
    )
    ...
    for step, (x, y) in enumerate(train_iter):
        ...
        if use_fp16:
            with torch.cuda.amp.autocast():
                out = model(
                    x.cuda().to(memory_format=torch.contiguous_format), y.cuda()
                )
                # out = model(x.cuda().to(memory_format=torch.contiguous_format))
        ...
        if step % config.train.freq_vis == 0 and not step == 0:
            ...
            print(
                "Epoch: {}; step: {}; loss: {:.4f}; acc: {:.4f}; lr: {}".format(
                    epoch, step, loss_avg, acc_avg, lr
                )
            )
...
def calc_roc_auc(val_loader, model, alpha=1, beta=0.2, thresh=0.85):
    ...
    for step, (img1, img2, score) in enumerate(val_iter):
        embeddings1 = (
            model(img1.cuda().to(memory_format=torch.contiguous_format)).cpu().numpy()
        )
        embeddings2 = (
            model(img2.cuda().to(memory_format=torch.contiguous_format)).cpu().numpy()
        )
    ...
    far = (
        np.count_nonzero(np.logical_and(total_scores == 0, preds == 1))
        / total_scores.size
    )
    frr = (
        np.count_nonzero(np.logical_and(total_scores == 1, preds == 0))
        / total_scores.size
    )
...
def validation(
    ...
    with torch.no_grad():
        ...
        if mode == "simple":
            ...
            print(
                "Validation of epoch: {} is done; \n rocauc: {:.4f}; acc: {:.4f}; far: {:.4f}; frr: {:.4f}".format(
                    epoch, rocauc, acc, far, frr
                )
            )
            return rocauc
